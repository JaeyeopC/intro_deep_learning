...
class MyOwnNetwork(ClassificationNet):
    ...
    def forward(self, X):
        ...
        for i in range(self.num_layer - 1):
            ...
            # Activation function
            X, cache_LeakyReLU = self.activation.forward(X)
            self.cache["leakyReLU" + str(i + 1)] = cache_LeakyReLU
    ...
    def backward(self, dy):
        ...
        for i in range(self.num_layer - 2, -1, -1):
            # Unpack cache
            cache_LeakyReLU = self.cache['LeakyReLU' + str(i + 1)]
            cache_affine = self.cache['affine' + str(i + 1)]

            # Activation backward
            dh = self.activation.backward(dh, cache_LeakyReLU)
